/**
 * QuickSwap Adapter
 * QuickSwap uses Uniswap V2 compatible contracts on Polygon
 */
...
class QuickSwapAdapter {
    ...
    async getAmountsOut(amountIn, path) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error getting amounts out: ${error.message}`));
            throw error;
        }
    }
    ...
    async getAmountsIn(amountOut, path) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error getting amounts in: ${error.message}`));
            throw error;
        }
    }
    ...
    async swapExactTokensForTokens(amountIn, amountOutMin, path, to, deadline, signer) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error executing swap: ${error.message}`));
            throw error;
        }
    }
    ...
    async swapTokensForExactTokens(amountOut, amountInMax, path, to, deadline, signer) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error executing reverse swap: ${error.message}`));
            throw error;
        }
    }
    ...
    /**
     * Get WETH (WMATIC on Polygon) address
     */
    async getWETH() {
    ...
    async addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, to, deadline, signer) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error adding liquidity: ${error.message}`));
            throw error;
        }
    }
    ...
    async removeLiquidity(tokenA, tokenB, liquidity, amountAMin, amountBMin, to, deadline, signer) {
        try {
            ...
            console.log(chalk.red(`QuickSwap: Error removing liquidity: ${error.message}`));
            throw error;
        }
    }
}
